...
def get_user_selections():
    ...
    console.print(
        create_question_box(
            "第1步：股票代码",
            "请输入要分析的精确股票代码，需要时请包含交易所后缀（示例：SPY, CNC.TO, 7203.T, 0700.HK）",
            "SPY",
        )
    )
    ...
    # Step 3: Market Region / Data Source
    console.print(
        create_question_box(
            "第3步：市场区域",
            "选择目标市场 — 决定数据来源（美股=yfinance，A股=akshare/Sina）"
        )
    )
    market_region = select_market_region()
...
def display_complete_report(final_state):
    ...
    if final_state.get("risk_debate_state"):
        ...
        if risk.get("judge_decision"):
            console.print(Panel("[bold]V. 投资组合经理决策[/bold]", border_style="green"))
...
def run_analysis(checkpoint: bool = False):
    ...
    # Apply market region / data source selection
    if selections.get("market_region") == "cn":
        config["data_vendors"] = {
    ...
    with Live(layout, refresh_per_second=_refresh_rate) as live:
        ...
        decision_raw = final_state.get("final_trade_decision",
            ...
            "## Analyst Summary\n\n(All downstream steps skipped — analyst reports below)"))
...
def analyze(
    checkpoint: bool = typer.Option(
        False,
        "--checkpoint",
        help="启用检查点/恢复：在每个节点后保存状态，以便崩溃时恢复运行。",
    ),
    clear_checkpoints: bool = typer.Option(
        False,
        "--clear-checkpoints",
        help="运行前删除所有已保存的检查点（强制全新开始）。",
    ),
):
    if clear_checkpoints:
        from tradingagents.graph.checkpointer import clear_all_checkpoints
        n = clear_all_checkpoints(DEFAULT_CONFIG["data_cache_dir"])
        console.print(f"[yellow]已清除 {n} 个检查点。[/yellow]")
